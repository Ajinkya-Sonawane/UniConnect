...
var InitialState = /** @class */ (function () {
    ...
    InitialState.prototype.assertCallCountMatchesExpectations = function (calls, // list of arguments
    callCount, type, // method or property
    property, args) {
        var expectedCount = this._expectedCount;
        this.clearExpectations();
        if (this.doesCallCountMatchExpectations(expectedCount, callCount))
            return;
        throw new Error('Expected ' + (expectedCount === null ? '1 or more' : expectedCount) +
            ' call' + (expectedCount === 1 ? '' : 's') + ' to the ' + type + ' ' + property.toString() +
            ' with ' + Utilities_1.stringifyArguments(args) + ', but received ' + (callCount === 0 ? 'none' : callCount) +
